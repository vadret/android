package fi.kroon.vadret.data.cache

import androidx.collection.LruCache
import fi.kroon.vadret.data.exception.Failure
import fi.kroon.vadret.data.functional.Either
import fi.kroon.vadret.data.weather.model.Weather
import fi.kroon.vadret.utils.DEFAULT_CACHE_KEY
...
class WeatherForecastCacheRepository @Inject constructor(
    ...
    fun getMemoryCache(): Single<Either<Failure, Weather>> =
        Single.fromCallable {
            memoryCache
                .snapshot()
                .getValue(DEFAULT_CACHE_KEY)
                .asRight() as Either<Failure, Weather>
        }.onErrorReturn {
            Failure.MemoryCacheLruReadFailure().asLeft()
        }
    ...
    fun updateMemoryCache(weather: Weather): Single<Either<Failure, Weather>> {
        memoryCache.put(DEFAULT_CACHE_KEY, weather)
        return Single.just(
            weather.asRight() as Either<Failure, Weather>
        ).onErrorReturn {
            Failure.MemoryCacheLruWriteFailure().asLeft()
        }
    }
    ...
    fun clearMemoryCache(): Single<Either<Failure, Unit>> = Single.fromCallable {
        ...
        Failure.DiskCacheEvictionFailure().asLeft()
    }
