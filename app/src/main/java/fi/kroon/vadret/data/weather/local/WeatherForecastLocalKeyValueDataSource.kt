package fi.kroon.vadret.data.weather.local
...
class WeatherForecastLocalKeyValueDataSource @Inject constructor(
    ...
    private val locationName: Pref<String> = rxkPrefs.string(LOCALITY_KEY, DEFAULT_FALLBACK_LOCALITY)
    private val municipalityName: Pref<String> = rxkPrefs.string(MUNICIPALITY_KEY, DEFAULT_FALLBACK_MUNICIPALITY)
    private val countyName: Pref<String> = rxkPrefs.string(COUNTY_KEY, DEFAULT_FALLBACK_COUNTY)
    private val latitude: Pref<String> = rxkPrefs.string(LATITUDE_KEY, DEFAULT_FALLBACK_LATITUDE)
    private val longitude: Pref<String> = rxkPrefs.string(LONGITUDE_KEY, DEFAULT_FALLBACK_LONGITUDE)
    private val automaticLocationMode: Pref<Boolean> = rxkPrefs.boolean(AUTOMATIC_LOCATION_MODE_KEY)
    ...
    fun getBoolean(key: String): Single<Either<Failure, Boolean>> =
        when (key) {
            AUTOMATIC_LOCATION_MODE_KEY -> Single.just(
                ...
                Single.just(WeatherForecastFailure.LoadingWeatherSettingFailed().asLeft())
            }
        }
    ...
    fun putBoolean(key: String, value: Boolean): Single<Either<Failure, Unit>> = when (key) {
        AUTOMATIC_LOCATION_MODE_KEY -> {
            ...
            Single.just(WeatherForecastFailure.CachingWeatherForecastDataFailed().asLeft())
        }
    }
    ...
    fun observeBoolean(key: String): Observable<Either<Failure, Boolean>> =
        when (key) {
            ...
            Either.Left(WeatherForecastFailure.LoadingWeatherSettingFailed())
        }
